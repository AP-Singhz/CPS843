...
try:
    ...
    while True:
        ...
        for barcode in decode(img):  # multiple barcodes
            ...
            # print(barcode.rect)
            try:
                Data = barcode.data.decode('utf-8')  # removes the string and gives actual data
                print(Data)
                ...
                cv2.putText(img, Data, (coords2[0], coords2[1]), cv2.FONT_HERSHEY_COMPLEX_SMALL, 2, (0, 0, 255), 2)  # puts barcode data on screen, (coords2[0], coords2[1]) is where text is drawn, 2 is font size, (0, 0, 255) is color (red), 2 is text thickness
